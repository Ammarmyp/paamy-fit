...
const ExerciseCard = (props) => {
  ...
  return (
    <div className="p-r rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap  ">
      <div className="flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4 ">
        <h4 className="text-3xl hidden sm:inline sm:text-4xl md:text-5xl font-semibold text-slate-400">
          0{i + 1}
        </h4>
        <h2 className="capitalize whitespace-nowrap truncate max-w-full text-lg sm:textxl md:text-2xl flex-1 md:text-center">
      ...
      <div className="flex flex-col bg-slate-950 rounded gap-2 ">
        {exercise.description.split("___").map((val) => {
          return <div className="text-sm" key={val} >{val}</div>;
        })}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2">
        {["reps", "rest", "tempo"].map((info) => {
          return (
            ...
            >
              <h3 className="capitalize text-slate-900 w-full">
